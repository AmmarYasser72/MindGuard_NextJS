...
import type { DoctorRecommendation, DoctorRecommendationResult, PatientConditionId } from "../types/recommendations";

export const DEFAULT_DOCTOR_CONDITION: PatientConditionId = "anxiety";

export function useDoctorRecommendations() {
  const [selectedCondition, setSelectedCondition] = useState<PatientConditionId>(DEFAULT_DOCTOR_CONDITION);
  const [result, setResult] = useState<DoctorRecommendationResult | null>(null);
  const [contactDoctor, setContactDoctor] = useState<DoctorRecommendation | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState("");
  const [requestVersion, setRequestVersion] = useState(0);
  const selectedConditionOption = useMemo(
    () => patientConditionOptions.find((condition) => condition.id === selectedCondition) || patientConditionOptions[0],
    [selectedCondition],
  );
  ...
  useEffect(() => {
    ...
    getDoctorRecommendations(selectedCondition)
      ...
      .catch((loadError) => {
        if (!isActive) return;
        setError(loadError instanceof Error ? loadError.message : "Unable to load doctor recommendations.");
        setResult(null);
      })
      .finally(() => {
        if (isActive) {
