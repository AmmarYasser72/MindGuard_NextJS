...
export function ensureApiData(response: unknown, label: string): unknown {
  ...
  if (!Object.prototype.hasOwnProperty.call(record, "data")) {
    throw createServiceError(
      `${label} endpoint returned no data yet.`,
      response,
    );
  }
  ...
  if (typeof record.data === "string") {
    throw createServiceError(
      `${label} endpoint returned placeholder text instead of data.`,
      response,
    );
  }
...
export function ensureArrayData<T = unknown>(
  response: unknown,
  label: string,
): T[] {
...
export function ensureObjectData<T extends ApiRecord = ApiRecord>(
  response: unknown,
  label: string,
): T {
  const data = ensureApiData(response, label);
...
export function ensureRecordHasAnyField<T extends ApiRecord = ApiRecord>(
  record: T,
  label: string,
  fields: string[],
): T {
  ...
  if (!fields.some((field) => hasFieldValue(record, field))) {
    throw createServiceError(
      `${label} endpoint returned no usable record data yet.`,
      record,
    );
  }
...
export function ensureArrayRecords<T extends ApiRecord = ApiRecord>(
  response: unknown,
  label: string,
  fields: string[],
): T[] {
