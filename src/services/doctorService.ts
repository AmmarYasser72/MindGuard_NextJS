import { ensureArrayRecords, ensureObjectData, ensureRecordHasAnyField } from "./apiResponse";
import { request } from "./apiClient";
import type { ApiRecord } from "../types/api";

const doctorFields = ["_id", "id", "specialization", "yearsOfExperience"];
const sessionSlotFields = ["_id", "id", "slots", "startTime", "endTime", "from", "to"];
...
export const doctorService = {
  ...
  async createSessionSlots({ from, to }: { from: string; to: string }) {
    ...
    return ensureRecordHasAnyField(ensureObjectData<ApiRecord>(response, "Doctor session slots"), "Doctor session slots", sessionSlotFields);
  },
};
