...
export default function PatientListButton({
  ...
  return (
    ...
    >
      {leading || (
        <span className="grid h-10 w-10 place-items-center rounded-2xl" style={{ backgroundColor: iconBg }}>
          <Icon name={icon} size={20} color={iconColor} />
        </span>
      )}
      <span className="min-w-0">
        {eyebrow ? <small className="block text-xs font-semibold text-slate-400">{eyebrow}</small> : null}
        <strong className="block text-sm font-bold text-slate-900">{title}</strong>
        {subtitle ? <small className="mt-1 block text-xs font-semibold text-slate-500">{subtitle}</small> : null}
        {meta ? <em className="mt-1 block text-xs not-italic leading-5 text-slate-400">{meta}</em> : null}
      </span>
