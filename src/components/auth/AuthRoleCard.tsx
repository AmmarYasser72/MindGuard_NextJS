...
export default function AuthRoleCard({ icon, onClick, subtitle, title, tone }: AuthRoleCardProps) {
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      <span className={`auth-field-icon grid h-12 w-12 place-items-center rounded-2xl ${styles.iconSurface}`}>
        <Icon name={icon} size={24} color={styles.icon} />
      </span>
      <span className="min-w-0">
        <strong className="block text-sm font-bold text-app-text">{title}</strong>
