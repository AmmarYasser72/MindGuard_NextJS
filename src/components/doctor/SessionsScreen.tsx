    ...
    <div className="grid w-full max-w-none gap-4 p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        ...
        {!isLoading && !error && !shown.length ? (
          <EmptyPanel
            message={
              tab === "past" ? "No past sessions" : "No upcoming sessions"
            }
          />
        ) : null}
      </div>
  ...
  return (
    ...
    >
      <div className="rounded-lg border border-red-100 bg-red-50 p-4 text-sm font-semibold leading-6 text-red-700">
        Delete the session with {session.patientName} on{" "}
        {formatDateTime(session.scheduledAt)}. This removes the slot from the
        doctor schedule.
      </div>
    </Modal>
  );
