...
export default function ScheduleSessionModal({ patient, onClose, onCreate }: ScheduleSessionModalProps) {
  const [form, setForm] = useState(() => ({ ...createDefaultScheduleForm(), patient: patient ? patientName(patient) : "Unassigned" }));
  ...
  async function submit() {
    ...
    try {
      ...
      const message = submitError instanceof Error ? submitError.message : "Unable to schedule session.";
      setError(message);
  ...
  return (
    <Modal title="Schedule New Session" onClose={onClose} actions={<><button type="button" className={secondaryButtonClass} onClick={onClose} disabled={isSubmitting}>Cancel</button><button type="button" className={primaryButtonClass} onClick={submit} disabled={isSubmitting}>{isSubmitting ? "Scheduling..." : "Schedule Session"}</button></>}>
