...
export default function DoctorHome({
  ...
  const segments = [
    {
      label: conditionLabels.anxiety,
      value: conditionDistribution.anxiety,
      color: "#6366f1",
    },
    {
      label: conditionLabels.stress,
      value: conditionDistribution.stress,
      color: "#8b5cf6",
    },
    {
      label: conditionLabels.depression,
      value: conditionDistribution.depression,
      color: "#ec4899",
    },
    {
      label: conditionLabels.mixed,
      value: conditionDistribution.mixed,
      color: "#10b981",
    },
    {
      label: "No significant condition",
      value: conditionDistribution.none,
      color: "#94a3b8",
    },
  ];
  ...
  const leadingPercent = totalPanel
          ...
          <div className="flex flex-col gap-4 border-b border-[var(--doctor-line)] bg-[linear-gradient(180deg,var(--doctor-card)_0%,var(--doctor-card-soft)_100%)] p-5 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 max-w-2xl text-sm font-medium leading-6 text-slate-500">
                Current clinical mix across the assigned patient panel.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">
                {upcoming.length
                  ? `Next ${upcoming.length} confirmed sessions`
                  : "No confirmed sessions for the next few hours"}
              </p>
            </div>
            <button
              type="button"
              className="grid h-10 w-10 place-items-center rounded-lg border border-violet-100 text-slate-600 transition hover:bg-violet-50 hover:text-[var(--primary)]"
