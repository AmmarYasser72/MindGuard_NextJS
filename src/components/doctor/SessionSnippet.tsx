...
export default function SessionSnippet({ onClick, session }) {
  ...
  const className =
    "grid gap-2 rounded-lg border border-[var(--doctor-line)] bg-[linear-gradient(180deg,var(--doctor-card)_0%,var(--doctor-card-soft)_100%)] p-4";

  if (onClick) {
    return (
      <button
        type="button"
        className={`${className} text-left transition hover:border-violet-200 hover:bg-violet-50/40 focus:outline-none focus:ring-4 focus:ring-violet-100`}
        onClick={onClick}
      >
        {content}
      </button>
