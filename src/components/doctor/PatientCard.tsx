...
export default function PatientCard({
  ...
  return (
    <article className="doctor-surface overflow-hidden rounded-lg border border-violet-100 bg-white shadow-sm shadow-violet-950/5">
      <button
        type="button"
        className="grid w-full grid-cols-[auto_1fr_auto] items-center gap-3 p-4 text-left transition hover:bg-violet-50/60"
        onClick={onToggle}
      >
        <span className="grid h-12 w-12 place-items-center rounded-lg bg-[var(--primary)] text-sm font-bold text-white shadow-sm shadow-violet-950/10">
          {initials(patient)}
        </span>
        <span className="min-w-0">
          <span className="flex flex-wrap items-center gap-2">
            <strong className="truncate text-base font-bold text-slate-950">
              {patientName(patient)}{" "}
              <span className="text-slate-500">{ageGender(patient)}</span>
            </strong>
          ...
          <em className="mt-1 flex items-center gap-2 text-xs font-semibold not-italic text-slate-400">
      ...
      {expanded ? (
        <div className="doctor-muted-panel grid gap-4 border-t border-violet-100 bg-violet-50/50 p-4">
          ...
          <p className="doctor-card-gradient rounded-lg border border-violet-100 bg-white p-3 text-sm font-medium text-slate-600">
            <strong className="text-slate-900">Last journal:</strong>{" "}
            {patient.journal
              ? `"${patient.journal.slice(0, 110)}..."`
              : "No recent entry"}
          </p>
