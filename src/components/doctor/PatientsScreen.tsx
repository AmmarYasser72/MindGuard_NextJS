...
import { filterButtonClass, primaryButtonClass, surfaceClass } from "./dashboardShared";
...
export default function PatientsScreen({ error, isLoading, onOpenSchedule, onRetry, patients = [] }: PatientsScreenProps) {
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState("sort");
  const [expanded, setExpanded] = useState<string | null>(null);
  const [detailPatient, setDetailPatient] = useState<DoctorPatient | null>(null);
  ...
  return (
    <div className="grid w-full max-w-none gap-4 p-4 sm:p-6 lg:p-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <button type="button" className={primaryButtonClass} onClick={() => onOpenSchedule(null)} disabled={isLoading}>
      ...
      <div className="grid gap-3">
        {isLoading ? <EmptyPanel message="Loading patients from the backend..." /> : null}
        ...
        {!isLoading && !error && !filtered.length ? <EmptyPanel message={patients.length ? "No patients match this search." : "No patients returned by the backend."} /> : null}
      </div>
