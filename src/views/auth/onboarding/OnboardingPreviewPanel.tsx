...
export default function OnboardingPreviewPanel({
  ...
  return (
    <aside
      className="order-1 mx-auto hidden w-full max-w-lg md:block lg:order-2"
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <span className="min-w-0">
                ...
                <small className="block truncate text-xs font-bold text-white/75">
                  Personalized onboarding
                </small>
              </span>
            </div>
            <span className="rounded-lg bg-white/15 px-3 py-2 text-xs font-bold">
              0{index + 1}
            </span>
          </div>
        </div>
...
function PreviewStat({ icon, label, tone, value }: PreviewStatProps) {
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm shadow-slate-950/5">
      ...
      <strong className="block text-lg font-bold text-slate-950 sm:text-xl">
        {value}
      </strong>
    </div>
  );
