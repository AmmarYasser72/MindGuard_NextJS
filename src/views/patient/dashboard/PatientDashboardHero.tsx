      ...
      <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_minmax(340px,430px)] lg:p-7">
        <div className="grid content-between gap-8">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <h1 className="mt-3 text-3xl font-bold tracking-normal sm:text-4xl">{greeting}, {patientName}</h1>
            ...
            <div className="patient-hero-actions flex shrink-0 flex-wrap justify-end gap-2">
              ...
              <button type="button" className={iconButtonClass} aria-label="Logout" title="Logout" onClick={onLogout}>
                <Icon name="log-out" size={20} color="#fff" />
              </button>
              <button type="button" className={iconButtonClass} aria-label="Refresh dashboard" title="Refresh dashboard" onClick={onRefresh} disabled={isRefreshing}>
                <Icon name="refresh-cw" size={20} color="#fff" className={isRefreshing ? "animate-spin" : ""} />
              </button>
              <button type="button" className={`${iconButtonClass} relative`} aria-label="Notifications" title="Notifications" onClick={onOpenNotifications}>
        ...
        <div className="rounded-lg border border-white/15 bg-white/10 p-4 shadow-sm shadow-indigo-950/10 backdrop-blur">
          ...
          <div className="mt-4 rounded-lg bg-white/10 p-3">
            ...
            <strong className="mt-1 block text-sm font-bold text-white">{nextGoalTitle}</strong>
          </div>
        </div>
      </div>
    </header>
