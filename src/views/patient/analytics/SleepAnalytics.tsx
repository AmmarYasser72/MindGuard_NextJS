  ...
  return (
    <div className="grid gap-5 p-4 sm:gap-6 sm:p-6">
      <AnalyticsHeader
        ...
        subtitle="Track your sleep quality and recovery patterns"
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          {sleepAnalytics.summary.map((item) => (
            <span className="grid min-w-[9rem] gap-2" key={item.label}>
              ...
              <small className="text-sm font-bold text-white/80">
                {item.label}
              </small>
      ...
      <Card className="grid gap-5 rounded-[1.5rem] p-5 sm:p-6">
        <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-start">
          <span className="grid gap-1.5">
            ...
            <small className="text-sm leading-6 text-slate-500">
              Comparing sleep quality with next-day energy shows how recovery
        ...
        <LineChart
          ...
          series={[
            {
              label: "Sleep Quality",
              data: sleepAnalytics.chart,
              color: "#8b5cf6",
              fill: true,
            },
            {
              label: "Energy Level",
              data: sleepAnalytics.energy,
              color: "#06b6d4",
              dash: "5 4",
              fill: false,
            },
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {sleepAnalytics.stages.map((stage) => (
            ...
            >
              ...
              <b className="text-2xl font-bold" style={{ color: stage.color }}>
                {stage.duration}
              </b>
      ...
      <Card className="grid gap-4 rounded-[1.5rem] p-5 sm:p-6">
        ...
        <div className="grid gap-3">
          {sleepAnalytics.tips.map((tip) => (
            <div
              className="patient-analytics-row grid gap-3 rounded-[1.25rem] border border-slate-200 bg-slate-50/80 p-4 sm:grid-cols-[auto_1fr] sm:items-center"
              key={tip.title}
            >
      ...
      <Card className="grid gap-4 rounded-[1.5rem] p-5 sm:p-6">
        ...
        <div className="grid gap-4">
          {sleepAnalytics.goals.map((goal) => (
            <GoalProgress key={goal.goal} goal={goal} />
          ))}
        </div>
      </Card>
    </div>
