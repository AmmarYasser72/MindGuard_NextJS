...
export default function DepressionAnalytics() {
  ...
  return (
    <div className="grid gap-5 p-4 sm:gap-6 sm:p-6">
      <AnalyticsHeader
        title="Depression"
        subtitle="Monitor your mental health patterns"
        timeframe={depressionAnalytics.timeframe}
      />
      <section className="grid gap-5 rounded-[1.75rem] bg-[linear-gradient(135deg,#ef4444,#dc2626)] px-6 py-6 text-white shadow-[0_22px_40px_rgba(185,28,28,0.16)]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <span className="grid min-w-[9rem] gap-2">
            <Icon name="brain" size={24} color="#fff" />
            <small className="text-sm font-bold text-white/80">Severity</small>
            <strong className="text-[1.75rem] font-bold leading-none">
              Mild
            </strong>
          </span>
          <span className="grid min-w-[9rem] gap-2">
            <Icon name="clipboard-list" size={24} color="#fff" />
            <small className="text-sm font-bold text-white/80">Score</small>
            <strong className="text-[1.75rem] font-bold leading-none">
              6/10
            </strong>
          </span>
          <span className="grid min-w-[9rem] gap-2">
            ...
            <small className="text-sm font-bold text-white/80">Trend</small>
            <strong className="text-[1.75rem] font-bold leading-none">
              Improving
            </strong>
          </span>
      ...
      <TechniqueSection
        ...
        techniques={depressionAnalytics.strategies}
        color="#ef4444"
      />
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {depressionAnalytics.correlations.map((item) => (
            ...
            >
              ...
              <b className="text-2xl font-bold" style={{ color: item.color }}>
                {item.value}
              </b>
        ...
        <div className="grid gap-3">
          {depressionAnalytics.support.map((item) => (
            <button
              ...
              onClick={() =>
                showToast(
                  `${item.title} resources are ready to open`,
                  "success",
                )
              }
            >
      ...
      <CrisisCard
        crisis={{
          ...
          primaryAction: {
            type: "toast",
            message:
              "If you feel unsafe, contact your local emergency or crisis service immediately.",
          },
          secondaryAction: {
            type: "navigate",
