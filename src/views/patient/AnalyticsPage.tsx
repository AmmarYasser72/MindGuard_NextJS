...
const analyticsMeta = {
  mood: {
    eyebrow: "Mood rhythm",
    headline:
      "Your emotional rhythm is steadier when check-ins happen early in the day.",
    subtitle:
      "A calmer view of how your emotions, streaks, and check-ins are trending across the week.",
    status: "Updated today",
    stats: [
      "12 day streak",
      "Morning mood strongest",
  ...
  stress: {
    ...
    headline:
      "Pressure spikes are visible now, so relief tools are easier to time well.",
    subtitle:
      "See how pressure changes through the day and which techniques lower your stress fastest.",
    status: "Live snapshot",
    stats: [
      "Low current level",
      "14 relief sessions",
      "Meetings are top trigger",
    ],
  ...
  sleep: {
    ...
    headline:
      "Rest quality and next-day energy are moving in the same healthy direction.",
    subtitle:
      "Understand how rest quality, bedtime rhythm, and recovery energy shape your next day.",
    status: "7 day window",
  ...
  depression: {
    ...
    headline:
      "Recovery signals are improving, with sleep and activity staying most connected.",
    subtitle:
      "Follow symptom trends, support patterns, and care signals in one focused recovery view.",
    status: "Weekly review",
    stats: [
      "Mild severity",
      "Improving trend",
      "Sleep has strongest correlation",
    ],
  ...
  anxiety: {
    ...
    headline:
      "You can see exactly when anxiety rises and which calming routines bring it down.",
    subtitle:
      "Track episodes, discover triggers, and jump into calming exercises when you need support.",
    status: "Today",
...
export default function AnalyticsPage() {
  ...
  const tab =
    analyticsTabs.find((item) => item.key === active) || analyticsTabs[0];
    ...
    <section className="patient-analytics-shell dashboard-analytics-shell min-h-screen w-full pb-24 pt-4 sm:pt-5 lg:pt-7">
        ...
        >
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <header className="max-w-3xl">
              <span className="text-xs font-black uppercase tracking-[0.22em] text-white/78">
                Patient insights
              </span>
              <h1 className="mt-3 text-[clamp(2.35rem,3.3vw,2.8rem)] font-normal leading-none tracking-[-0.05em]">
                {tab.title}
              </h1>
              <p className="mt-3 max-w-2xl text-base leading-7 text-white/92">
                {meta.headline}
              </p>
            </header>
          ...
          <div className="relative grid content-center gap-5 py-6 lg:grid-cols-[minmax(0,1fr)_390px] lg:gap-8 lg:py-3">
            <div className="grid content-start gap-5">
              ...
              <div>
                ...
                <p className="mt-4 max-w-[40rem] text-base font-medium leading-7 text-white/75">
                  {meta.subtitle}
                </p>
              </div>
            </div>
          ...
          <nav
            className="relative grid gap-3 sm:grid-cols-2 lg:grid-cols-5"
            aria-label="Patient analytics categories"
            role="tablist"
          >
            {analyticsTabs.map((item) => (
              <button
                ...
                className={cn(
                  ...
                  item.key === active &&
                    "border-white/22 shadow-[0_14px_30px_rgba(15,23,42,0.18)]",
                )}
                ...
                style={
                  item.key === active
                    ? {
                        background: `linear-gradient(135deg, ${item.color} 0%, rgba(255,255,255,0.16) 170%)`,
                      }
                    : undefined
                }
              ...
              >
                <span
                  className={`grid h-11 w-11 shrink-0 place-items-center rounded-[1rem] ${item.key === active ? "bg-white/24" : "bg-white/14"}`}
                >
                  <Icon
                    name={item.icon}
                    size={21}
                    color={item.key === active ? "#fff" : item.color}
                  />
                </span>
                <span className="grid min-w-0 gap-1">
                  ...
                  <small className="truncate text-xs font-black text-white/75 sm:whitespace-normal">
                    {item.title}
                  </small>
                </span>
              </button>
      ...
      <div className="mx-auto mt-5 w-full max-w-[1240px] px-4 sm:px-6 xl:px-0">
        ...
        >
          ...
          {active === "stress" ? (
            <StressLikeAnalytics data={stressAnalytics} />
          ) : null}
          {active === "anxiety" ? (
            <StressLikeAnalytics data={anxietyAnalytics} />
          ) : null}
