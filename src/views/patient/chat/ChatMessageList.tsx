  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-4 pt-1 md:px-8 md:pb-6 xl:px-12" ref={listRef}>
      <div className="grid w-full gap-5">
        {!hasUserMessage ? prompts.map((prompt) => (
          <button
            type="button"
        ...
        {messages.map((message, index) => (
          <ChatBubble message={message} key={`${message.text}-${message.time}-${index}`} />
        ))}
...
function TypingCard() {
  return (
    <article className="patient-chat-ai-bubble mr-auto flex w-fit items-center gap-1.5 rounded-[0.85rem] border-2 border-[#7b61d8] bg-white px-5 py-4 shadow-[0_16px_30px_rgba(84,61,145,0.08)]" aria-label="AI is typing">
