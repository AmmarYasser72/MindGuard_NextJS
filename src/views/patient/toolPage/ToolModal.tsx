...
import { ActivityLogForm, ExerciseDetail, WorkoutPlanner } from "./ExerciseModals";
import GenericDetail from "./GenericDetail";
import { JournalDetail, JournalEntryForm, MoodCheckForm } from "./JournalModals";
...
export default function ToolModal({ activeModal, color, onClose, onFinish, onSaveJournal, onSaveSleep, toolTitle }: ToolModalProps) {
  const item = activeModal.item as ToolItem;
  const itemTitle = item.title || toolTitle;

  if (activeModal.kind === "breathing-player") {
    return <BreathingPlayer item={item} color={color} onClose={onClose} onFinish={() => onFinish(`${itemTitle} completed.`)} />;
  }
  ...
  if (activeModal.kind === "exercise-detail") {
    return <ExerciseDetail item={item} onClose={onClose} onStart={() => onFinish(`${itemTitle} plan started.`)} />;
  }
