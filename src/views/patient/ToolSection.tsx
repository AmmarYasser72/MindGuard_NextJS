  ...
  return (
    <section className="grid gap-3">
      ...
      {section.type === "journal" ? (
        <JournalEntries items={section.items} onItem={onAction} />
      ) : null}
      {section.type === "goals" ? (
        <GoalList items={section.items} onItem={onAction} />
      ) : null}
      {section.type === "activity" ? (
        <ActivityRows items={section.items} onItem={onAction} />
      ) : null}
      {section.type === "breakdown" ? (
        <Card>
          <BreakdownGrid items={section.items} />
        </Card>
      ) : null}
      {section.type === "days" ? (
        <Card>
          <DaysRow days={section.days} active={section.active} />
        </Card>
      ) : null}
      {section.type === "sleep" ? (
